import { _decorator, CCFloat, CCInteger, Component, Layout, screen, Size, UITransform, Widget } from 'cc';
import { Utils } from '../Utils/Utils';
const { ccclass, property } = _decorator;

@ccclass('LayoutRightManager')
export class LayoutRightManager extends Component {
    @property(Widget) widget: Widget = null!;
    @property(CCInteger) defaultRight: number = 30;
    @property(Layout) layoutCheck: Layout;
    @property(CCFloat) scale: number = 0.9;
    start() {
        let scaleWindow = Utils.getScaleWindow();
        let isFitHeight: boolean = Utils.isFitHeight();
        if (isFitHeight) {
            this.widget.right = this.defaultRight - (scaleWindow - 1) * 720 / 2;
        }
