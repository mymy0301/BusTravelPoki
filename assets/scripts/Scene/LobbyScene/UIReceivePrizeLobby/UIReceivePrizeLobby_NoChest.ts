...
import { GameSoundEffect, IPrize, PAGE_VIEW_LOBBY_NAME, TYPE_CURRENCY, TYPE_PRIZE, TYPE_RECEIVE_PRIZE_LOBBY } from '../../../Utils/Types';
...
import { PageHomeSys } from '../PageSys/PageHomeSys';
import { UILobbySys } from '../UILobbySys';
const { ccclass, property } = _decorator;

@ccclass('UIReceivePrizeLobby_NoChest')
export class UIReceivePrizeLobby_NoChest extends UIReceivePrizeLobbyBase {
    ...
    @property([Node]) listNLayout: Node[] = [];
    ...
    override async Play(): Promise<void> {
        ...
        for (let index = 0; index < actionPrize.data.length; index++) {
            ...
            // const nLayout = this.GetLayout(index, 4);
            // nLayout.addChild(itemPrize);
    ...
    public async AnimShowUI(cbDone: CallableFunction) {
        ...
        SoundSys.Instance.playSoundEffectOneShotDelayTime(GameSoundEffect.SOUND_REWARDS_SHOW, 1, 0.1);
        // convert dataCustom to ActionReceivePrizeClass
        const actionPrize = this._dataCustom;
        let nameAnim: string = "";
        let nameAnimIdleOpen: string = "";
        //======================= anim ========================
        const scaleItem = MConfigs.GetScaleItem(actionPrize.data.length);
    ...
    //#region self func
    private UpdateAllLayout() {
        this.listNLayout.forEach(nLayout => {
            let listLayout: Layout[] = nLayout.getComponents(Layout);
            nLayout.scale = nLayout.scale.clone();
            listLayout.forEach((layout, index) => {
                layout.spacingX = 20;
            })
        })
    }
    ...
    private async onBtnContinue() {
        ...
        const pageHomeNow = UILobbySys.Instance?.GetIndexPageShow();
        ...
        for (let i = 0; i < this._listPrizes.length; i++) {
            ...
            switch (true) {
                case itemCom.prizeData.typePrize == TYPE_PRIZE.TICKET && (pageHomeNow == PAGE_VIEW_LOBBY_NAME.HOME):
                    ...
                    break;
                case itemCom.prizeData.typePrize == TYPE_PRIZE.MONEY && (pageHomeNow == PAGE_VIEW_LOBBY_NAME.HOME):
                    ...
                    await Utils.WaitReceivingDone(() => { return canNextLogic })
                    break;
                case pageHomeNow == PAGE_VIEW_LOBBY_NAME.HOME:
                    ...
                    await Utils.WaitReceivingDone(() => { return canNextLogic })
                    break;
                case itemCom.prizeData.typePrize == TYPE_PRIZE.TICKET && (pageHomeNow == PAGE_VIEW_LOBBY_NAME.SHOP):
                    clientEvent.dispatchEvent(MConst.EVENT_SHOP.GET_WPOS_UI_TICKET, (wPosUI: Vec3) => {
                        canNextLogic = true;
                        wPosEnd = wPosUI;
                    })
                    await Utils.WaitReceivingDone(() => { return canNextLogic })
                    break;
                case itemCom.prizeData.typePrize == TYPE_PRIZE.MONEY && (pageHomeNow == PAGE_VIEW_LOBBY_NAME.SHOP):
                    clientEvent.dispatchEvent(MConst.EVENT_SHOP.GET_WPOS_UI_COIN, (wPosUI: Vec3) => {
                        canNextLogic = true;
                        wPosEnd = wPosUI;
                    })
                    await Utils.WaitReceivingDone(() => { return canNextLogic })
                    break;
                case pageHomeNow == PAGE_VIEW_LOBBY_NAME.SHOP:
                    clientEvent.dispatchEvent(MConst.EVENT_LOBBY.GET_WPOS_TAB_HOME, (wPosUI: Vec3) => {
                        canNextLogic = true;
                        wPosEnd = wPosUI;
                    })
                    await Utils.WaitReceivingDone(() => { return canNextLogic })
