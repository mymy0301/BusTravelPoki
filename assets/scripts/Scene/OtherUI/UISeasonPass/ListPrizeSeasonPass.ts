...
@ccclass('ListPrizeSeasonPass')
export class ListPrizeSeasonPass extends Component {
    ...
    private _nLockPrize: Node = null;
    private _sizeItem: Size = null;

    public async SetUp(data: InfoPrizePass[], nLockPrize: Node) {
        this._nLockPrize = nLockPrize;
    ...
    private async SetItem(item: Node, info: InfoPrizePass, index: number) {
        const itemCom = item.getComponent(ItemPrizeSeasonPass);
        itemCom.SetUpCallbackReceivePrizeNormal(this.PlayAnimReceivePrizeNormal.bind(this));
        if (info.index == 0) {
            // await itemCom.SetUpUIIndex0(info, this.itemPrize, this.sfBgLvLock, this.sfBgLvUnlock, this.nPrizeSpecial);
        ...
        } else if (info.index == 4 || info.index == 9 || info.index == 14 || info.index == 19 || info.index == 24 || info.index == 29) {
    ...
    public async ScrollToTheFirstItemWasUnlock(isFirstInit: boolean = false) {
        ...
        for (let i = 0; i < this.contentSV.children.length; i++) {
            const itemCom = this.contentSV.children[i].getComponent(ItemPrizeSeasonPass);
            ...
            if (itemCom != null && itemCom.GetStateFree() == STATE_ITEM_PRIZE_SEASON_PASS.CAN_NOT_CLAIM && indexLevelUnlockNow > i) {
    ...
    public async unlockAllItemPremium() {
        ...
        const levelPlayerNow = DataSeasonPassSys.Instance.GetLevelNow().level;
        ...
        for (let i = 0; i < this.contentSV.children.length; i++) {
            const itemCom = this.contentSV.children[i].getComponent(ItemPrizeSeasonPass);
            if (itemCom != null) {
                listPromise.push(itemCom.unlockSeasonPassPremium(levelPlayerNow));
            }
        }
    ...
    private UpdateLocLockPrize() {
        ...
        // if (DataSeasonPassSys.Instance.CanClaimLastPrize()) {
        ...
        let levelReach = DataSeasonPassSys.Instance.GetLevelNow().level;
