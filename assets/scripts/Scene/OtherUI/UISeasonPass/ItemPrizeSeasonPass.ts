import { _decorator, CCFloat, Color, Component, instantiate, Label, Node, Prefab, Sprite, SpriteFrame, Tween, tween, UIOpacity, UITransform, Vec2, Vec3 } from 'cc';
import { clientEvent } from '../../../framework/clientEvent';
import { MConst, TYPE_UI } from '../../../Const/MConst';
import { FBInstantManager } from '../../../Utils/facebooks/FbInstanceManager';
import { InfoPrizePass, IPrize, TYPE_CURRENCY, TYPE_PRIZE } from '../../../Utils/Types';
...
import { EVENT_SEASON_PASS, SEASON_PASS_DESCRIBE_NOTI } from './TypeSeasonPass';
...
export enum STATE_ITEM_PRIZE_SEASON_PASS {
    CAN_NOT_CLAIM,
    WAIT_TO_CLAIM,
    CLAIMED
}
...
export function GetNameStateItemPrizeSeasonPass(state: STATE_ITEM_PRIZE_SEASON_PASS) {
    switch (state) {
        case STATE_ITEM_PRIZE_SEASON_PASS.CAN_NOT_CLAIM:
            return 'CAN_NOT_CLAIM';
        case STATE_ITEM_PRIZE_SEASON_PASS.WAIT_TO_CLAIM:
            return 'WAIT_TO_CLAIM';
        case STATE_ITEM_PRIZE_SEASON_PASS.CLAIMED:
            return 'CLAIMED';
        default:
            return 'null';
    }
...
@ccclass('ItemPrizeSeasonPass')
export class ItemPrizeSeasonPass extends Component {
    ...
    private _stateFree: STATE_ITEM_PRIZE_SEASON_PASS = STATE_ITEM_PRIZE_SEASON_PASS.CAN_NOT_CLAIM;
    private _statePremium: STATE_ITEM_PRIZE_SEASON_PASS = STATE_ITEM_PRIZE_SEASON_PASS.CAN_NOT_CLAIM;
    ...
    public SetUpUIChest(info: InfoPrizePass, itemPrize: Prefab, sfChestFree: SpriteFrame
        ...
        switch (info.index) {
            ...
            case 14: indexChest = 1; break;
            case 19: indexChest = 1; break;
            case 24: indexChest = 1; break;
            case 29: indexChest = 1; break;
     ...
     */
    public async unlockSeasonPassPremium(levelPlayerNow: number) {
        // just update state of premium again 
        let isClaimed = DataSeasonPassSys.Instance.IsClaimPrizeAtLevel(this._infoItem.index);
        if (levelPlayerNow > this._infoItem.index) {
            ...
            this.UpdateState(1, isClaimed.premium ? STATE_ITEM_PRIZE_SEASON_PASS.CLAIMED : STATE_ITEM_PRIZE_SEASON_PASS.WAIT_TO_CLAIM);
        } else {
            this.UpdateState(1, STATE_ITEM_PRIZE_SEASON_PASS.CAN_NOT_CLAIM);
        }
    }
    private BaseSetUp(info: InfoPrizePass, itemPrize: Prefab, sfBgLvLock: SpriteFrame, sfBgLvUnlock: SpriteFrame) {
        ...
        const isBuyPass = DataSeasonPassSys.Instance.IsActivePass();

        // set label
        this.lbLevel.string = info.index.toString();

        // check the level of player is reach 
        const levelPlayer = DataSeasonPassSys.Instance.GetLevelNow();
        let levelPlayerNow = levelPlayer.level;
        const progressShow = levelPlayer.progressBar;

        // console.log("check data", levelPlayer);


        // in special case last prize
        if (levelPlayerNow == MConfigs.MAX_PRIZE_SEASON_PASS && info.index == MConfigs.MAX_PRIZE_SEASON_PASS - 1) {
            // maybe can claim or was claim
            let isClaimed = DataSeasonPassSys.Instance.IsClaimPrizeAtLevel(info.index);
            this.UpdateState(0, isClaimed.free ? STATE_ITEM_PRIZE_SEASON_PASS.CLAIMED : STATE_ITEM_PRIZE_SEASON_PASS.WAIT_TO_CLAIM);
            this.UpdateState(1, isClaimed.premium ? STATE_ITEM_PRIZE_SEASON_PASS.CLAIMED : (isBuyPass ? STATE_ITEM_PRIZE_SEASON_PASS.WAIT_TO_CLAIM : STATE_ITEM_PRIZE_SEASON_PASS.CAN_NOT_CLAIM));
            ...
            this.UpdateState(0, STATE_ITEM_PRIZE_SEASON_PASS.CAN_NOT_CLAIM);
            this.UpdateState(1, STATE_ITEM_PRIZE_SEASON_PASS.CAN_NOT_CLAIM);
     ...
     * @param newState 
     */
    public UpdateState(index: 0 | 1, newState: STATE_ITEM_PRIZE_SEASON_PASS) {
        if (index == 0) {
    ...
    public CanReceivePrizeFreeOrPremium(): boolean {
        if (this._stateFree == STATE_ITEM_PRIZE_SEASON_PASS.WAIT_TO_CLAIM || this._statePremium == STATE_ITEM_PRIZE_SEASON_PASS.WAIT_TO_CLAIM) {
            return true;
        }
        return false;
    }
    ...
    public GetPrizeFreeIfCanReceive(): IPrize[] {
        if (this._stateFree == STATE_ITEM_PRIZE_SEASON_PASS.WAIT_TO_CLAIM) {
    ...
    public GetPrizePremiumIfCanReceive(): IPrize[] {
        if (this._statePremium == STATE_ITEM_PRIZE_SEASON_PASS.WAIT_TO_CLAIM) {
            return this._infoItem.listItemsPassPremium;
        }
        return [];
    }

    public GetStateFree(): STATE_ITEM_PRIZE_SEASON_PASS { return this._stateFree; }
    ...
    private async ClaimItemFree() {
        ...
        switch (this._infoItem.index) {
            ...
            case 14: indexChest = 0; break;
            case 19: indexChest = 0; break;
            case 24: indexChest = 0; break;
            case 29: indexChest = 0; break;
        }

        //save received prize item
        DataSeasonPassSys.Instance.SaveClaimPrizeAtLevel(this._infoItem.index, 'free', true);

        if (indexChest != -1) {
            PrizeSys.Instance.AddPrize(DataSeasonPassSys.Instance.GetListPrizeAtLevel(this._infoItem.index).free, 'SeasonPass_ClaimPrizeFree', true, false);
            // the receive chest in lobby already claim prize
            clientEvent.dispatchEvent(EVENT_SEASON_PASS.RECEIVE_CHEST, this._infoItem.listItemsPassFree, 0, 'SeasonPass_ClaimPrizeFree');
        } else {
            ...
            PrizeSys.Instance.AddPrize(DataSeasonPassSys.Instance.GetListPrizeAtLevel(this._infoItem.index).free, 'SeasonPass_ClaimPrizeFree');
    ...
    private async ClaimItemPremium() {
        ...
        //save received prize item
        DataSeasonPassSys.Instance.SaveClaimPrizeAtLevel(this._infoItem.index, 'prenium', true);

        if (indexChest != -1 && this._infoItem.index != 0) {
            PrizeSys.Instance.AddPrize(DataSeasonPassSys.Instance.GetListPrizeAtLevel(this._infoItem.index).premium, 'SeasonPass_ClaimPrizePremium', true, false);
            // the receive chest in lobby already claim prize
            clientEvent.dispatchEvent(EVENT_SEASON_PASS.RECEIVE_CHEST, this._infoItem.listItemsPassPremium, indexChest, 'SeasonPass_ClaimPrizePremium');
        } else {
            //================== play anim ================
            ...
            // call save claim prize in here
            // save the prize was claimed => do not call save in this func because in the func addPrize it was call save data
            PrizeSys.Instance.AddPrize(DataSeasonPassSys.Instance.GetListPrizeAtLevel(this._infoItem.index).premium, 'SeasonPass_ClaimPrizePremium');
            await this._cbPlayAnimPrizeNormal(visualPrize, wPos);
        }
    }
    //#endregion
    ...
    private onBtnItemFree() {
        LogEventManager.Instance.logButtonClick(`free_prize_${GetNameStateItemPrizeSeasonPass(this._stateFree)}`, "ItemUISeasonPass");
        ...
        if (this._stateFree == STATE_ITEM_PRIZE_SEASON_PASS.WAIT_TO_CLAIM) {
            ...
            // if (DataSeasonPassSys.Instance.GetNumPrizeCanClaim() == 1) {
            //     this.ClaimItemFree();
            //     this.UpdateState(0, STATE_ITEM_PRIZE_SEASON_PASS.CLAIMED);
            // } else {
            //     clientEvent.dispatchEvent(EVENT_SEASON_PASS.RECEIVE_LIST_ITEM_PRIZE);
            // }

            this.ClaimItemFree();
            this.UpdateState(0, STATE_ITEM_PRIZE_SEASON_PASS.CLAIMED);
        } else if (this._stateFree == STATE_ITEM_PRIZE_SEASON_PASS.CAN_NOT_CLAIM) {
            const iCustomBubble: ICustomBubble = {
                ar: [0, 0, 35, 0],
            }

            if (index == 4 || index == 9 || index == 14 || index == 19 || index == 24 || index == 29) {
                clientEvent.dispatchEvent(EVENT_SEASON_PASS.NOTIFICATION.ITEMS,
                ...
                clientEvent.dispatchEvent(EVENT_SEASON_PASS.NOTIFICATION.TEXT
                    , SEASON_PASS_DESCRIBE_NOTI.NOTIFI_CAN_NOT_COLLECTED_FREE
                    , TYPE_BUBBLE.BOTTOM_LEFT
                    , wLocShowNotification
                    , true
                    , this.node.parent
            ...
            clientEvent.dispatchEvent(EVENT_SEASON_PASS.NOTIFICATION.TEXT
                , SEASON_PASS_DESCRIBE_NOTI.NOTIFI_HAS_COLLECTED
                , TYPE_BUBBLE.BOTTOM_LEFT
                , wLocShowNotification
                , true
                , this.node.parent
    ...
    private onBtnItemPremium() {
        LogEventManager.Instance.logButtonClick(`prenium_prize_${GetNameStateItemPrizeSeasonPass(this._statePremium)}`, "ItemUISeasonPass");
        ...
        if (this._statePremium == STATE_ITEM_PRIZE_SEASON_PASS.WAIT_TO_CLAIM) {
            ...
            //     this.UpdateState(1, STATE_ITEM_PRIZE_SEASON_PASS.CLAIMED);
            // } else {
            //     clientEvent.dispatchEvent(EVENT_SEASON_PASS.RECEIVE_LIST_ITEM_PRIZE);
            // }
            ...
            this.ClaimItemPremium();
            this.UpdateState(1, STATE_ITEM_PRIZE_SEASON_PASS.CLAIMED);
        } else if (this._statePremium == STATE_ITEM_PRIZE_SEASON_PASS.CAN_NOT_CLAIM) {
            if (index == 4 || index == 9 || index == 14 || index == 19 || index == 24 || index == 29) {
                clientEvent.dispatchEvent(EVENT_SEASON_PASS.NOTIFICATION.ITEMS
            ...
            clientEvent.dispatchEvent(EVENT_SEASON_PASS.NOTIFICATION.TEXT
                , isBuyPass ? SEASON_PASS_DESCRIBE_NOTI.NOTIFI_HAS_COLLECTED : SEASON_PASS_DESCRIBE_NOTI.NOTIFI_CAN_NOT_COLLECTED_PREMIUM
                , TYPE_BUBBLE.BOTTOM_RIGHT
                , wLocShowNotification
                , true
                , this.node.parent
    ...
    private onBtnLevel() {
        LogEventManager.Instance.logButtonClick(`level`, "ItemUISeasonPass");

        const wLocShowNotification = this.lbLevel.node.worldPosition.clone();
        if (this._stateFree == STATE_ITEM_PRIZE_SEASON_PASS.CAN_NOT_CLAIM) {
            const iCustomBubble: ICustomBubble = {
                bb: [30, 0, 0, 0],
            }
