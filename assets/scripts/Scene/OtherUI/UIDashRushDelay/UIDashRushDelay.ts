 ...
 * db://assets/scripts/Scene/OtherUIUIDashRushDelay/UIDashRushDelay.ts
*
*/
...
import { STATE_DR } from '../UIDashRush/TypeDashRush';
const { ccclass, property } = _decorator;

@ccclass('UIDashRushDelay')
export class UIDashRushDelay extends UIBaseSys {
    @property(Label) lbContent: Label;
    @property(Label) lbTime: Label;
    private readonly contentFailed: string = 'You failed the challenge.\n';
    private readonly contentDefault: string = 'Next Dash Rush will start in:';
    //==========================================
    //#region base
    public async PrepareDataShow(): Promise<void> {
        ...
        if (stateEvent == STATE_DR.DELAY_LOSE) {
    ...
    private EndCoolTimeByGroup(typeEventOff: TYPE_EVENT_GAME) {
        if (typeEventOff == TYPE_EVENT_GAME.DASH_RUSH) {
    ...
    //#region btn
    public OnBtnClose() {
        LogEventManager.Instance.logButtonClick(`close`, "UIDashRushDelay");
        clientEvent.dispatchEvent(MConst.EVENT.CLOSE_UI, TYPE_UI.UI_DASH_RUSH_DELAY, 1);
    }
    //#endregion btn
    //==========================================
}