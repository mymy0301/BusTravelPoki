import { _decorator, Color, Component, easing, ITweenOption, Label, Node, random, randomRangeInt, Sprite, tween, Tween, TweenEasing, UIOpacity, Vec3 } from 'cc';
import { CONFIG_HAT_RACE } from './TypeHatRace';
const { ccclass, property } = _decorator;

@ccclass('HRProgressSys')
export class HRProgressSys extends Component {
    ...
    private readonly _baseScale: Vec3 = new Vec3(1.1, 1.1, 1.1);
    ...
    private _indexNow: number = 0; public IndexNow() { return this._indexNow; }
    ...
    public MoveToIndex(indexMoveTo: number) {
        if (this._indexNow == -1 && indexMoveTo == 0) {
            this.ShowItemAtProgress0();
            return;
        }
    ...
    private GetListTweenAnimMoveToIndex(indexMoveTo: number) {
        ...
        if (indexMoveTo < 0 || indexMoveTo > this.nBaseNode.length - 1 || this._indexNow == indexMoveTo) return null;
        ...
        listTween.push(tween(this.nBig)
            ...
            .parallel(
                ...
                tween().to(dataMove.timeMoveEnd, { position: dataMove.ePos, scale: this._baseScale })
            )
    ...
    private GetAllPosBetween2Index(indexStart: number, indexEnd: number): { sPos: Vec3, listMidPos: Vec3[], listTimeMid: number[], timeMoveEnd: number, ePos: Vec3 } {
        ...
        switch (true) {
            ...
            case distanceIndex >= 1:
                ...
                return {
                    sPos: new Vec3(posStart.x, -1.871),
                    listMidPos: resultMidPos,
                    listTimeMid: isFromLeftToRight ? resultTime : resultTime.reverse(),
                    timeMoveEnd: timeMoveLast,
                    ePos: new Vec3(posEnd.x - 3, -1.871)
                }
        }
    }

    private SetLbIndex(index: number, needTween: boolean = true) {
        if (index < 0 || index > CONFIG_HAT_RACE.MULTIPLIER.length) { this.lbBig.node.active = false; return; }

        this.lbBig.node.active = true;
        if (!needTween) {
            this.lbBig.string = `x${CONFIG_HAT_RACE.MULTIPLIER[index]}`;
            return;
        }
    ...
    private ShowItemAtProgress0() {
        this.lbBig.string = `x${CONFIG_HAT_RACE.MULTIPLIER[0]}`;
        const self = this;
        const opaCom = self.nBig.getComponent(UIOpacity);

        this.nBig.active = true;
        tween(this.nBig)
            .call(() => {
                self.nBig.scale = Vec3.ZERO;
                opaCom.opacity = 0;
                self.SetLbIndex(0, false);
            })
            .to(0.3, { scale: this._baseScale }, {
