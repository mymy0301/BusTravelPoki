...
import { AnimPackHalloween } from '../../../AnimsPrefab/Halloween/AnimPackHalloween';
import { DataInfoPlayer } from '../../DataInfoPlayer';
const { ccclass, property } = _decorator;

@ccclass('UIPackHalloween')
export class UIPackHalloween extends UIBaseSys {
    ...
    @property(AnimPackHalloween) animPackHalloween: AnimPackHalloween = null;
    ...
    public async PrepareDataShow(): Promise<void> {
        this._infoPack = DataHalloweenSys.Instance.InfoPackHalloweenWorking;
        if (this._infoPack == null) { this.node.active = false; return; }

        // update bg and ske
        if (!this._wasSetData) {
            this._wasSetData = true;
            this.animPackHalloween.SetData(MConfigResourceUtils._skeHalloween);
            const indexPackHalloween = DataHalloweenSys.Instance.GetIndexPackNow();
            switch (indexPackHalloween) {
                case 0:
                    this.animPackHalloween.PlayPack1();
                    break;
                case 1:
                    this.animPackHalloween.PlayPack2();
                    break;
                case 2:
                    this.animPackHalloween.PlayPack3();
                    break;
                case 3:
                    this.animPackHalloween.PlayPack4();
                    break;
                default:
                    this.animPackHalloween.PlayPack1();
                    break;
    ...
    public async UIShowDone(): Promise<void> {
        ...
        if (this._infoPack == null) { clientEvent.dispatchEvent(MConst.EVENT.CLOSE_UI, TYPE_UI.UI_PACK_HALLOWEEN, 2) }
    }
    ...
    private OnBtnClose() {
        ...
        clientEvent.dispatchEvent(MConst.EVENT.CLOSE_UI, TYPE_UI.UI_PACK_HALLOWEEN, 1);
    }
