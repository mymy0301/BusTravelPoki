...
@ccclass('LogicTutEventEndlessTreasure')
export class LogicTutEventEndlessTreasure extends Component {
    ...
    public JustCheckTut() {
        const logic1 = DataEventsSys.Instance.IsLockEvent(TYPE_EVENT_GAME.ENDLESS_TREASURE);
        const logic2 = DataEventsSys.Instance.IsPlayTutorialEvent(TYPE_EVENT_GAME.ENDLESS_TREASURE);
        const logic3 = this.node.active == true;
        const logic4 = DataEventsSys.Instance._listTypeEventShow.includes(TYPE_EVENT_GAME.ENDLESS_TREASURE);
    ...
    private CheckLogic_1(): boolean {
        ...
        if (valid) {
            clientEvent.dispatchEvent(UITutLobby.EVENT_TUT_SHOW, TYPE_TUT_EVENT_LOBBY.TUT_ENDLESS_TREASURE, this.node);
    ...
    private CheckLogicDone_1() {
        ...
        if (valid) {
            ...
            // init new event
            DataEndlessTreasureSys.Instance.InitNewEvent();
            DataEventsSys.Instance.SetPlayedTutorialEvent(TYPE_EVENT_GAME.ENDLESS_TREASURE);
            clientEvent.dispatchEvent(UITutLobby.EVENT_TUT_END, TYPE_TUT_EVENT_LOBBY.TUT_ENDLESS_TREASURE, this.node);
        }
    }
    //#endregion TUT 1
    ...
    private CheckLogic_2(): boolean {
        const valid = this.JustCheckTut();
        if (valid) {
            clientEvent.dispatchEvent(MConst.EVENT.SHOW_UI, TYPE_UI.UI_TUTOIRAL_IN_LOBBY, 2, true, TYPE_EVENT_GAME.ENDLESS_TREASURE);
            return true;
        }
        return false;
    }
    ...
    private CheckLogicDone_2() {
        ...
        if (valid) {
            ...
            DataEndlessTreasureSys.Instance.InitNewEvent();
            DataEventsSys.Instance.SetPlayedTutorialEvent(TYPE_EVENT_GAME.ENDLESS_TREASURE);
            clientEvent.dispatchEvent(MConst.EVENT_GAME.UPDATE_TIME, TYPE_EVENT_GAME.ENDLESS_TREASURE);
        }
    }
    //#endregion TUT 2
}
