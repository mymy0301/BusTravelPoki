...
import { CONFIG_SR } from '../../OtherUI/UISpeedRace/TypeEventSpeedRace';
const { ccclass, property } = _decorator;

@ccclass('LogicTutEventSpeedRace')
export class LogicTutEventSpeedRace extends Component {
    ...
    public JustCheckTut() {
        const logic1 = DataEventsSys.Instance.IsLockEvent(TYPE_EVENT_GAME.SPEED_RACE);
        const logic2 = DataEventsSys.Instance.IsPlayTutorialEvent(TYPE_EVENT_GAME.SPEED_RACE);
        const logic3 = this.node.active == true;
        const logic4 = DataEventsSys.Instance._listTypeEventShow.includes(TYPE_EVENT_GAME.SPEED_RACE);
    ...
    private CheckLogic_1(): boolean {
        ...
        if (valid) {
            clientEvent.dispatchEvent(UITutLobby.EVENT_TUT_SHOW, TYPE_TUT_EVENT_LOBBY.TUT_SPEED_RACE, this.node);
    ...
    private CheckLogicDone_1() {
        ...
        if (valid) {
            ...
            DataEventsSys.Instance.SetPlayedTutorialEvent(TYPE_EVENT_GAME.SPEED_RACE);
            clientEvent.dispatchEvent(UITutLobby.EVENT_TUT_END, TYPE_TUT_EVENT_LOBBY.TUT_SPEED_RACE, this.node);
        }
    }
    //#endregion TUT 1
    ...
    private CheckLogic_2(): boolean {
        const valid = this.JustCheckTut();
        if (valid) {
            clientEvent.dispatchEvent(MConst.EVENT.SHOW_UI, TYPE_UI.UI_TUTOIRAL_IN_LOBBY, 2, true, TYPE_EVENT_GAME.SPEED_RACE);
            return true;
        }
        return false;
    }
