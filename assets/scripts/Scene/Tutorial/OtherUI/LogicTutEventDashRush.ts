...
@ccclass('LogicTutEventDashRush')
export class LogicTutEventDashRush extends Component {
    ...
    public JustCheckTut() {
        const check1: boolean = DataEventsSys.Instance.IsLockEvent(TYPE_EVENT_GAME.DASH_RUSH);
        const check2: boolean = DataEventsSys.Instance.IsPlayTutorialEvent(TYPE_EVENT_GAME.DASH_RUSH);
        const check3: boolean = this.node.active == true;
        const check4: boolean = DataEventsSys.Instance._listTypeEventShow.includes(TYPE_EVENT_GAME.DASH_RUSH);
    ...
    private CheckLogic_1(): boolean {
        ...
        if (valid) {
            clientEvent.dispatchEvent(UITutLobby.EVENT_TUT_SHOW, TYPE_TUT_EVENT_LOBBY.TUT_DASH_RUSH, this.node);
            return true;
        }
        return false;
    }
    ...
    private CheckLogicDone_1() {
        ...
        if (valid) {
            ...
            DataEventsSys.Instance.SetPlayedTutorialEvent(TYPE_EVENT_GAME.DASH_RUSH);
            clientEvent.dispatchEvent(UITutLobby.EVENT_TUT_END, TYPE_TUT_EVENT_LOBBY.TUT_DASH_RUSH, this.node);
        }
    }
    //#endregion TUT 1
    ...
    private CheckLogic_2(): boolean {
        const valid = this.JustCheckTut();
        if (valid) {
            clientEvent.dispatchEvent(MConst.EVENT.SHOW_UI, TYPE_UI.UI_TUTOIRAL_IN_LOBBY, 2, true, TYPE_EVENT_GAME.DASH_RUSH);
            return true;
        }
        return false;
    }
    ...
    private CheckLogicDone_2() {
        ...
        if (valid) {
            DataEventsSys.Instance.SetPlayedTutorialEvent(TYPE_EVENT_GAME.DASH_RUSH);
            this.nEventName.active = false;
            if (DataEventsSys.Instance.IsEventShowButLock(TYPE_EVENT_GAME.DASH_RUSH)) {
