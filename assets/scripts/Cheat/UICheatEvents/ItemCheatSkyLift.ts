...
@ccclass('ItemCheatSkyLift')
export class ItemCheatSkyLift extends Component {
    ...
    //#region time
    private UpdateTime() {
        const timeRemaining = DataSkyLiftSys.Instance.GetTimeDisplay();
        if (timeRemaining <= 0) {
    ...
    private UpdateTime_delay() {
        const timeRemaining = DataSkyLiftSys.Instance.GetTimeDisplay_Delay();
        if (timeRemaining <= 0) {
    ...
    private OnBtnChangeTimeCooldown() {
        try {
            ...
            DataSkyLiftSys.Instance.ForceChangeTimeCooldown(timeChange);
        } catch (e) {
            console.error(e);
        }
    }
    ...
    private OnBtnChangeTimeDelay() {
        try {
            ...
            const timeChange = Number.parseInt(input);
            if (timeChange < 0) { throw (`time is negative`) }
            DataSkyLiftSys.Instance.ForceChangeTimeDelay(timeChange);
        } catch (e) {
            console.error(e);
        }
    }
