...
@ccclass('ItemCheatDashRush')
export class ItemCheatDashRush extends Component {
    ...
    //#region time
    private UpdateTime() {
        const timeRemaining = DataDashRush.Instance.GetTimeDisplay();
        if (timeRemaining <= 0) {
    ...
    private UpdateTime_delay() {
        const timeRemaining = DataDashRush.Instance.GetTimeDisplay_Delay();
        if (timeRemaining <= 0) {
    ...
    private OnBtnChangeTimeCooldown() {
        try {
            ...
            DataDashRush.Instance.ForceChangeTimeCooldown(timeChange);
        } catch (e) {
            console.error(e);
        }
    }
    ...
    private OnBtnChangeTimeDelay() {
        try {
            ...
            const timeChange = Number.parseInt(input);
            if (timeChange < 0) { throw (`time is negative`) }
            DataDashRush.Instance.ForceChangeTimeDelay(timeChange);
        } catch (e) {
            console.error(e);
        }
    }
