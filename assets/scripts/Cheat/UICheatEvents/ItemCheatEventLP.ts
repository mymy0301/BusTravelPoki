...
import { EVENT_LEVEL_PASS } from '../../Scene/OtherUI/UILevelPass/TypeLevelPass';
const { ccclass, property } = _decorator;

@ccclass('ItemCheatEventLP')
export class ItemCheatEventLP extends Component {
    ...
    //#region time
    private UpdateTime() {
        const timeRemaining = CaculTimeEvents2.Instance.GetTimeEvent(TYPE_EVENT_GAME.LEVEL_PASS);
        if (timeRemaining <= 0) {
    ...
    private UpdateTimeDelay() {
        const timeRemaining = CaculTimeEvents2.Instance.GetTimeDelay(TYPE_EVENT_GAME.LEVEL_PASS);
        if (timeRemaining <= 0) {
    ...
    public OnBtnSetTime() {
        try {
            ...
            CaculTimeEvents2.Instance.ForceChangeTimeEvent(TYPE_EVENT_GAME.LEVEL_PASS, timeChange);
        } catch (e) {
            console.error(e);
        }
    }
    ...
    public OnBtnSetTimeDelay() {
        try {
            ...
            if (input == '') { throw ('empty input') }
            const timeChange = Number.parseInt(input);
            CaculTimeEvents2.Instance.ForceChangeTimeDelayEvent(TYPE_EVENT_GAME.LEVEL_PASS, timeChange);
        } catch (e) {
            console.error(e);
        }
    }
