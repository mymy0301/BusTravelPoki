...
export class oneToMultiListener {
    ...
    public static isOnEvent(eventName: string, handler: Function, target: any): boolean {
        // check case not found handle in list
        if (this.handlers[eventName] == null || this.handlers[eventName] == undefined || this.handlers[eventName].length == 0) {
            return false;
        } else {
            for (let i = 0; i < this.handlers[eventName].length; i++) {
                const oldObj = this.handlers[eventName][i];
                if (oldObj.handler === handler && (target && target === oldObj.target)) {
                    return true;
                }
            }
    ...
    public static TryRemoveAllListenerHasEventName(eventName: string) {
        const handlerList = this.handlers[eventName];
        if (!handlerList) {
            return;
        }
        handlerList.slice(0, handlerList.length);
    }
};
